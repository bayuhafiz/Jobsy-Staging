        ...
        async.waterfall([
            function(done) {
                User.findOne({
                    ...
                    if (existingUser) {
                        req.flash('error', 'Sorry! Account with that email address already exists.');
                        res.redirect('/');
                    }
            ...
            function(token, done) {
                ...
                user.image = 'dummy.png';
                user.password = user.generateHash(req.body.password);
                user.actToken = token;
                user.actTokenCreated = Date.now();
                user.actTokenExpired = Date.now() + 3600000; // 1 hour
                user.actStatus = 'inactive';
            ...
            function(token, user, done) {
                ...
                smtpTransport.sendMail(mailOptions, function(err) {
                    req.flash('success', 'Your account has been created. An e-mail has been sent to ' + user.email + ' with further instructions.');
                    done(err, 'done');
                })
